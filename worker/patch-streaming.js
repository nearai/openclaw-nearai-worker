#!/usr/bin/env node
// Workaround: NEAR AI streaming bug truncates tool call arguments (see NEARAI_STREAMING_BUG.md).
// pi-ai hardcodes stream:true; the params.streaming workaround doesn't reach here (openclaw#12218).
//
// This patch replaces the streaming API call with a non-streaming call, then synthesizes
// the streaming events that openclaw's internal pipeline expects. Tool call arguments
// arrive complete in non-streaming mode, avoiding the truncation bug entirely.
...
const replacement = `// PATCHED: non-streaming to avoid NEAR AI tool call truncation bug
