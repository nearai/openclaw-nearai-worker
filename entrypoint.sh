#!/bin/bash
set -eu -o pipefail

# Security: Prevent accidental exposure of sensitive environment variables
# Never log, echo, or print the values of these variables:
# - NEARAI_API_KEY
# - SLACK_BOT_TOKEN
# - SLACK_APP_TOKEN
# - MOLTBOT_GATEWAY_TOKEN
# - SLACK_SIGNING_SECRET
#
# Only log variable names in error messages, never their values.
#
# WARNING: Do not enable debug mode (set -x) as it will expose all variable values
# in the shell output. If debugging is needed, use explicit echo statements
# that only print variable names, not values.

# Validate required environment variables
if [ -z "${NEARAI_API_KEY:-}" ]; then
  echo "Error: NEARAI_API_KEY environment variable is required" >&2
  exit 1
fi

if [ -z "${SLACK_BOT_TOKEN:-}" ]; then
  echo "Error: SLACK_BOT_TOKEN environment variable is required" >&2
  exit 1
fi

if [ -z "${SLACK_APP_TOKEN:-}" ]; then
  echo "Error: SLACK_APP_TOKEN environment variable is required" >&2
  exit 1
fi

if [ -z "${MOLTBOT_GATEWAY_TOKEN:-}" ]; then
  echo "Error: MOLTBOT_GATEWAY_TOKEN environment variable is required" >&2
  exit 1
fi

# Create config directory if it doesn't exist
mkdir -p /home/node/.moltbot

# Generate config from template if it doesn't exist or if forced
# Set MOLTBOT_FORCE_CONFIG_REGEN=1 to force regeneration even if config exists
FORCE_REGEN="${MOLTBOT_FORCE_CONFIG_REGEN:-0}"
if [ ! -f /home/node/.moltbot/moltbot.json ] || [ "${FORCE_REGEN}" = "1" ]; then
  if [ "${FORCE_REGEN}" = "1" ]; then
    echo "Force regenerating config from template (MOLTBOT_FORCE_CONFIG_REGEN=1)..."
  else
    echo "Generating config from template..."
  fi
  
  # Template file must exist
  if [ ! -f /app/moltbot.json.template ]; then
    echo "Error: Template file /app/moltbot.json.template not found" >&2
    exit 1
  fi
  
  # Export variables for envsubst (only the ones we need)
  export NEARAI_API_KEY
  export SLACK_BOT_TOKEN
  export SLACK_APP_TOKEN
  export MOLTBOT_GATEWAY_TOKEN
  
  # Use envsubst to substitute environment variables in the template
  # Moltbot supports ${VAR_NAME} syntax natively, so we can use the template directly
  if command -v envsubst >/dev/null 2>&1; then
    envsubst < /app/moltbot.json.template > /home/node/.moltbot/moltbot.json
  else
    echo "Error: envsubst command not found (gettext-base package required)" >&2
    exit 1
  fi
  
  chown node:node /home/node/.moltbot/moltbot.json
  echo "Config file created at /home/node/.moltbot/moltbot.json"
fi

# Create workspace directory if it doesn't exist
mkdir -p /home/node/clawd
chown -R node:node /home/node/clawd

# Setup SSH if SSH_PUBLIC_KEY is provided
if [ -n "${SSH_PUBLIC_KEY:-}" ]; then
  echo "Setting up SSH access..."
  mkdir -p /home/node/.ssh
  echo "${SSH_PUBLIC_KEY}" > /home/node/.ssh/authorized_keys
  chmod 700 /home/node/.ssh
  chmod 600 /home/node/.ssh/authorized_keys
  chown -R node:node /home/node/.ssh
  echo "SSH public key configured"
fi

# Start SSH server in background if SSH_ENABLED is set
# Port 2222 is non-privileged, so node user can bind to it
if [ "${SSH_ENABLED:-0}" = "1" ]; then
  echo "Starting SSH server on port 2222..."
  mkdir -p /tmp
  /usr/sbin/sshd -f /etc/ssh/sshd_config -D &
  SSH_PID=$!
  sleep 1
  if kill -0 $SSH_PID 2>/dev/null; then
    echo "SSH server started successfully on port 2222 (PID: $SSH_PID)"
  else
    echo "Warning: SSH server failed to start. Check SSH configuration."
  fi
fi

# Execute the command (moltbot is installed globally)
exec "$@"
