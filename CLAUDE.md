...
- `ssh-bastion/` — SSH bastion service: Alpine image with sshd that authenticates users by instance name (via compose-api) and proxies to worker containers. See `ssh-bastion/entrypoint.sh`, `lookup-keys.sh`, `connect-worker.sh`.
...
- **SSH bastion** — A single sshd on port 2222 proxies SSH connections to worker instances. Users connect as `ssh -p 2222 <instance-name>@<host>`. The bastion authenticates via `AuthorizedKeysCommand` (queries compose-api for the user's SSH key), then hops to the worker using an internal ed25519 key pair (`ForceCommand`). The bastion generates its key pair on first start (persisted in `management-data` volume at `/app/data/bastion/`). Compose-api reads the bastion's public key and injects it as `BASTION_SSH_PUBKEY` into worker containers so they accept the bastion's connections. The bastion polls compose-api every 5s to sync system users (sshd requires `/etc/passwd` entries for each instance name).
